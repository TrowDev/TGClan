...
public class Metrics {
	...
	static {
		...
		if (System.getProperty("bstats.relocatecheck") == null
			...
			final String defaultPackage = new String(new byte[] { 'o', 'r',
					'g', '.', 'b', 's', 't', 'a', 't', 's', '.', 'b', 'u', 'k',
					'k', 'i', 't' });
			final String examplePackage = new String(new byte[] { 'y', 'o',
					'u', 'r', '.', 'p', 'a', 'c', 'k', 'a', 'g', 'e' });
	...
	private JSONObject getServerData() {
		...
		try {
			...
			playerAmount = onlinePlayersMethod.getReturnType().equals(
					Collection.class) ? ((Collection<?>) onlinePlayersMethod
					.invoke(Bukkit.getServer())).size()
					: ((Player[]) onlinePlayersMethod
							.invoke(Bukkit.getServer())).length;
		} catch (Exception e) {
			playerAmount = Main.pl.getOnlinePlayers().length; // Just use the
																// new method if
																// the
																// Reflection
																// failed
		...
		for (Class<?> service : Bukkit.getServicesManager().getKnownServices()) {
			try {
				...
				for (RegisteredServiceProvider<?> provider : Bukkit
					...
					try {
						pluginData.add(provider.getService()
	...
	private static void sendData(Plugin plugin, JSONObject data)
		...
		if (logSentData) {
			plugin.getLogger().info(
					"Sending data to bStats: " + data.toString());
		}
	...
	public static class DrilldownPie extends CustomChart {
		...
		public JSONObject getChartData() throws Exception {
			...
			for (Map.Entry<String, Map<String, Integer>> entryValues : map
				...
				boolean allSkipped = true;
				for (Map.Entry<String, Integer> valueEntry : map.get(
						entryValues.getKey()).entrySet()) {
